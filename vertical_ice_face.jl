...
h_param = (
            ...
            a = - 5.72e-2, # ⁰C psu⁻¹, Freezing point coefficient
            b = 9.39e-2, # ⁰C, Freezing point coefficient
...
@inline vᵢₙᵢₜ(x, y, z) = @inbounds 1e-6
@inline wᵢₙᵢₜ(x, y, z) = @inbounds 1e-6

# Temperature initial condition: a stable density gradient with random noise superposed.
@inline Tᵢₙᵢₜ(x, y, z) = h_param.Tᵢₙᵢₜ + 1e-4 * Ξ(x)
@inline Sᵢₙᵢₜ(x, y, z) = h_param.Sᵢₙᵢₜ + 1e-4 * Ξ(x)

# `set!` the `model` fields using functions or constants:
set!(model, v=vᵢₙᵢₜ, w=wᵢₙᵢₜ, S=Sᵢₙᵢₜ, T=Tᵢₙᵢₜ)
...
wizard = TimeStepWizard(cfl=0.5, max_change=1.01, max_Δt=1second)
